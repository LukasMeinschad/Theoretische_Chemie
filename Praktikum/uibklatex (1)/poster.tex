...
% - BASED ON:    Originally developed by Jakob M. Messner,
%                adapted and adjusted to meet the UIBK CI in 2017.
% - DATE:        2021-02-01
% -------------------------------------------------------------------
...
% Important: when using the poster template for printing (e.g., make sure your
% final PDF is of version 1.4.  Newer versions can cause problems when printing
% (Studia/print shop). The following forces pdflatex to use PDF version 1.4;
% has no effect when using xelatex or luatex!
% For XeTeX users:
% * https://tex.stackexchange.com/questions/202300/options-controlling-pdf-output
% * or comment in the Makefile
% For LuaTeX users:
% * Comment the following three lines
% * Uncomment \directlua{pdf.setminorversion(4)}
% Check pdf version using 'pdfinfo <pdffile>'
\makeatletter
  \@ifundefined{pdfminorversion}{}{\pdfminorversion=4}
\makeatother
